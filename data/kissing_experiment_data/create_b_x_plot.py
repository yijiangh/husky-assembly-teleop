...
def plot_offsets_bx(data, out_path: Path | None = None):
    ...
    plt.scatter(data_non_stalled["x"], data_non_stalled["b"], color="red", label="Failure")
    plt.scatter(data_stalled["x"], data_stalled["b"], color="green", label="Success")
    plt.legend(loc='upper left')
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.xlabel("X Offset [mm]")
    plt.ylabel("B Offset [deg]")
    plt.title("B X Offset Plot")
    plt.tight_layout()
    if out_path:
