...
def get_sample_files(dir_path: Path):
    return sorted(dir_path.glob("dual_offset_*.json"))
...
def extract_data(records):
    ...
    for data in records:
        ...
        stalled.append(bool(data.get("motor_stalled_left")))
        trajectory_finished.append(bool(data.get("trajectory_finished_left")))
        raw_wrench_profile = data.get("wrench_profile_left")
...
def plot_force_profiles(data, title, out_path: Path | None = None):
    ...
    data = data.assign(
        ...
        tc_distance=lambda df: df["x"] + df["y"] + 0.0001
    )
