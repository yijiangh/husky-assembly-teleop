...
def traj_label(file_name, fallback_idx=0):
    """Extract the 'J<n>_traj<m>' label out of a calibration filename.

    e.g. "..._0806_J0_traj3_JointTrajectory.json" -> "J0_T3".
    The (\d+) capture groups grab the J number and traj number so we can
    rebuild the label as "J<x>_T<y>". Falls back to "T<idx>" if no token.
    """
    m = re.search(r'J(\d+)_traj(\d+)', file_name or '')
...
def analyze_base_mocap_variation(takes):
    ...
    take_labels = []      # Trajectory id (e.g. "J0_traj3") for each take, from its filename

    for take in takes:
        take_boundaries.append(len(all_base_mocap_pos))  # Record start index of this take
        take_labels.append(traj_label(take.get('file_name', ''), len(take_labels)))
...
def visualize_base_mocap_variation(mocap_data, joint_name, output_dir):
    ...
    # Add vertical separators for each data take, labelled by trajectory id
    take_boundaries = mocap_data.get('take_boundaries', [])
    take_labels = mocap_data.get('take_labels', [])
    for i, boundary in enumerate(take_boundaries[1:], start=1):  # Skip first boundary (index 0)
        ...
        # boundary i marks the start of take i -> use that take's label (e.g. "J0_traj2")
        ax.text(boundary, ax.get_ylim()[1] * 0.95, take_labels[i], fontsize=8, ha='center', va='top', color='gray', rotation=90)
