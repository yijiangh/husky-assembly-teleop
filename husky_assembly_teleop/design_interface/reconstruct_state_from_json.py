...
robot_cell_state_folder = os.path.join(
    design_study_path,
    design_case,
    "RobotCellStates",
)
start_state_file = os.path.join(robot_cell_state_folder, "robotx_box_A0-G_start_state_RobotCellState.json")
end_state_file = os.path.join(robot_cell_state_folder, "robotx_box_A0-G_RobotCellState.json")

with open(start_state_file, "r") as f:
    start_state = json_load(f)

with open(end_state_file, "r") as f:
    end_state = json_load(f)
...
with PyBulletClient(connection_type="gui" if use_gui else "direct", verbose=False) as client:
    ...
    # Robot base pose need to be manually updated
    robot_base_frame = end_state.robot_base_frame
    robot_base_pose = pose_from_frame(robot_base_frame)
    pp.set_pose(client.robot_puid, robot_base_pose)
