...
def build_obstacles(table_top_z: float = TABLE_TOP_Z,
    ...
    Args:
        table_top_z (float): Height of the placeholder table top [m].
...
def unwrap_goal(goal_q12, start_q12) -> np.ndarray:
    ...
    manages inside the time budget (documented at husky_world.py:2115). UR5e
    joint limits are well past 2*pi, so the unwrapped goal is always reachable.
    ...
    return goal - np.round((goal - start) / (2.0 * np.pi)) * 2.0 * np.pi
...
def plan_approach(robot, attachments, obstacles, start_q12, goal_q12,
    ...
    joints = pp.joints_from_names(robot, JOINT_NAMES_12)
    start = np.asarray(start_q12, dtype=float)
    goal = unwrap_goal(goal_q12, start)
    wrapped = int(np.sum(np.abs(goal - np.asarray(goal_q12, dtype=float)) > 1e-6))
    if wrapped:
        log(f'[approach] unwrapped {wrapped} goal joint(s) to the branch '
            f'nearest the start')
